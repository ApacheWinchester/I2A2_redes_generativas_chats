...
function getGreeting() {
  ...
  if (currentHour >= 5 && currentHour < 12) {
    return "<strong>Dr.Briza: Good morning!</strong>Hello, take a seat, make yourself comfortable. Start by telling me a little about yourself!";
  } else if (currentHour >= 12 && currentHour < 18) {
    return "<strong>Dr.Briza:Good afternoon!</strong> Hello, take a seat, make yourself comfortable. Start by telling me a little about yourself!";
  } else {
    return "<strong>Dr.Briza: Goodnight!</strong> Hello, take a seat, make yourself comfortable. Start by telling me a little about yourself!";
  }
}
...
function dialog() {
  ...
  var capitalizedInput = input.charAt(0).toUpperCase() + input.slice(1); // Capitaliza a primeira letra
  document.Eliza.input.value = "";

  const userMessage = `<div class="message incoming"><strong>Você:</strong> ${capitalizedInput}</div>`;
  chatter.push(userMessage);

  elizaresponse = listen(capitalizedInput);
...
function respond() {
  const elizaMessage = `<div class="message outgoing"><strong>Dr.Eliza:</strong> ${elizaresponse}</div>`;
