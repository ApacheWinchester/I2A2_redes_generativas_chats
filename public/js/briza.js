...
maxKey = 38; // Atualizado para o número correto de keywords em português
...
maxresponses = 120; // O número total de respostas após adicionar as keywords em português
...
// ... (restante do código, como definido anteriormente)

// Função para escolher uma resposta aleatória diferente das anteriores
function getRandomResponse(keyidx) {
  idxmin = keyword[keyidx].idx;
  idrange = keyword[keyidx].end - idxmin + 1;
  
  var availableChoices = [];
  for (var i = idxmin; i <= keyword[keyidx].end; i++) {
    if (!keyword[keyidx].used.includes(i)) {
  ...
  if (availableChoices.length === 0) {
    keyword[keyidx].used = [];
    availableChoices.push(keyword[keyidx].idx);
  }
  
  var choice = availableChoices[Math.floor(Math.random() * availableChoices.length)];
  keyword[keyidx].used.push(choice);
  
  if (keyword[keyidx].used.length >= 3) {
    keyword[keyidx].used.shift(); // remove the oldest entry if more than 3 stored
  }
    ...
    if (foundkey == keyNotFound) {
      if (!greet) { greet = true; return "É bom estar conversando com você." }
        ...
        if ((sInput.length < 10) && (wTopic != "") && (wTopic != wPrevious)) {
          ...
          return 'Sobre isso... "' + lTopic + '". me fale mais.';
        } else {
          if (sInput.length < 8) {
...
//novos
keyword[38] = new key("OBRIGADO", 120, 120);
...
response[3] = "Claro que posso,  me conte mais!";
response[4] = "Não estou convencido(a) de que você realmente deseje fazer isso. <*"
response[5] = "Hmm... você deseja ser capaz de realizar isso? <*"
...
// novos 
response[120] = "Todos nós fazemos uma imagem de nós mesmo, nos olhamos em um espelho e vocêe está se olhando no espelho errado, vendo uma imagem distorcida de si mesmo. Procure se olhar da maneira correta,enxergar o potencial que existe em você, você nasceu para ser feliz, invista em você mesmo, em algo que goste. Enquanto manter essa imagem negativa verá os contratempos como grandes montanhas,mude de atitude , reconheça seu potencial. Procure ver as situaçoes com outros olhos,como desafios a serem vencidos.Você é rapaz é uma pessoa rica em ideiase de grandes atitudes."
...
function getGreeting() {
	...
	if (currentHour >= 5 && currentHour < 12) {
	  return "<strong>Dr.Briza: Bom dia!</strong> Olá, se acomode, sinta-se à vontade. Comece me contando um pouco sobre você!";
	} else if (currentHour >= 12 && currentHour < 18) {
	  return "<strong>Dr.Briza: Boa tarde!</strong> Olá, se acomode, sinta-se à vontade. Comece me falando um pouco sobre você!";
	} else {
	  return "<strong>Dr.Briza: Boa noite!</strong> Olá, se acomode, sinta-se à vontade. Comece me contando um pouco sobre você!";
	}
  }
